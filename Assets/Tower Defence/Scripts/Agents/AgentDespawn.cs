...
public class AgentDespawn : MonoBehaviour
{
    [SerializeField] private float timeDelay;      //time waited before platforms get destroyed
	...
	void Update()
	{
		...
		if (timeCurrent > timeDelay)            //only activates after the set time delay
		{
			...
			timeCurrent -= timeDelay;      //resets the timer
		}
